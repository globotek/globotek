<?php
/**
 * Created by PhpStorm.
 * User: wmjc1
 * Date: 25/01/2019
 * Time: 22:48
 */ ?>

<div class="hero">
	
	<div class="hero__background">
		
		<img class="hero__background__image" src="<?php echo get_template_directory_uri() . '/images/hero-home-bg.svg'; ?>"/>
	
	</div>
	
	<div class="hero__inner">
		
		<div class="hero__cta">
			
			<div class="hero__cta__content">
				
				<h2 class="hero__title title title__primary">Our Online Portfolio</h2>
				
				<div class="hero__text">
					<p>Lorem ipsum dolor sit amet adipiscing elit. Curabitur et vestibulum arcu.</p>
				</div>
				
				<a class="hero__link button button--large button--white" href="#">Browse Our Work</a>
			
			</div>
			
			<div class="hero__cta__image">
				<img src="<?php echo get_template_directory_uri() . '/images/people-and-cloud.png'; ?>"/>
			</div>
		
		</div>
	
	</div>

</div>
